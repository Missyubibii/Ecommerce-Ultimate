...
<div class="bg-white rounded-xl shadow-lg overflow-hidden">
    ...
    <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center bg-gray-50">
        ...
        <div class="flex gap-2">
            <a href="{{ route('admin.products.index') }}" class="px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm font-medium">Quay lại</a>
            <a href="{{ route('admin.products.edit', $product) }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 text-sm font-medium">Chỉnh sửa</a>
        </div>
    </div>
        ...
        <div class="lg:col-span-1 p-6 space-y-6">
            ...
            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200 space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="px-2 py-1 text-xs font-bold rounded-full {{ $product->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-200 text-gray-800' }}">
                        {{ ucfirst($product->status) }}
                    </span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">Giá bán</span>
                    <span class="text-xl font-bold text-indigo-600">{{ number_format($product->price, 0, ',', '.') }} đ</span>
                </div>
                ...
                <div class="flex justify-between items-center text-sm">
                    ...
                    <span class="font-medium {{ $product->quantity <= $product->min_stock ? 'text-red-600' : 'text-gray-900' }}">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <tbody class="divide-y divide-gray-200 bg-white">
                                @foreach($product->metadata['specs'] as $spec)
                                    <tr>
                                        <td class="px-4 py-3 text-sm font-medium text-gray-500 w-1/3 bg-gray-50">{{ $spec['key'] }}</td>
