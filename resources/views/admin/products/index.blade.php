...
@section('content')
    <div x-data="productIndexPage(@js($initialData))" class="p-6 bg-white rounded-xl shadow-lg">
            ...
            <div class="flex flex-wrap items-center gap-4">
                ...
                <div class="flex-1 min-w-[200px]">
                    <div class="relative">
                        ...
                        <input type="text" x-model="search" @keyup.enter="applyFilters()" placeholder="Tìm theo tên, SKU..."
        ...
        <div x-show="selectedProducts.length > 0" x-transition
            ...
            <div class="flex items-center">
                <span class="text-sm font-semibold text-indigo-800">Đã chọn <span x-text="selectedProducts.length"></span>
                    sản phẩm</span>
            </div>
            <div class="flex items-center space-x-2">
                ...
                <button @click="selectedProducts = []; selectAll = false;"
                    class="bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-bold py-1 px-3 rounded text-sm transition duration-300">
                    Hủy
                </button>
            </div>
        </div>
                        ...
                        @forelse ($products as $product)
                            <tr class="hover:bg-gray-50 transition group">
                                        ...
                                        <div class="h-10 w-10 flex-shrink-0 mr-3">
                                            @if($product->image)
                                                <img src="{{ $product->image_url }}" alt="{{ $product->name }}"
                                                    class="h-10 w-10 rounded object-cover mr-3 border border-gray-200">
                                            @else
                                                <div
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @else
                                        <span
                                            class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-600">Draft</span>
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-right space-x-2">
                                    <a href="{{ route('admin.products.show', $product) }}"
                                        class="text-indigo-600 hover:text-indigo-900 font-medium text-xs uppercase tracking-wider">Xem</a>
                                    <a href="{{ route('admin.products.edit', $product) }}"
                                        class="text-blue-600 hover:text-blue-900 font-medium text-xs uppercase tracking-wider">Sửa</a>
                                </td>
                            </tr>
                        @empty
                            <tr>
